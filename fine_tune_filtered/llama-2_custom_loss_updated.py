...
def custom_loss_function(outputs, labels, specific_token_ids=[27208, 19361, 744]):
    ...
    for i in range(batch_size):
        start_idx = op_pos[i]
        end_idx = start_idx + 9
        # Extract the logits for the range op_pos[i]:op_pos[i] + 9 for this batch element
        logits_slice = logits[i, start_idx:end_idx, :]
        extracted_logits.append(logits_slice)
    
    extracted_logits = torch.stack(extracted_logits)  # (batch_size, 9, num_classes)

    softmax_logits = torch.nn.functional.softmax(extracted_logits, dim=-1)

    

    masked_probs = softmax_logits * one_hot.unsqueeze(0)
...
import json
file_path = 'fine_tuned_llama2_test_custom_loss_r8_a64_tok_32000_new_format_loss_v1.json'

with open(file_path, 'w') as file:
    json.dump(loss, file, indent=4) 


trainer.model.save_pretrained('./fine_tuned_llama2_test_custom_loss_r8_a64_tok_32000_new_format_loss_v1')
tokenizer.save_pretrained('./fine_tuned_llama2_test_custom_loss_r8_a64_tok_32000_new_format_loss_v1')
...
log_history_df.to_json('LOG____fine_tuned_llama2_test_custom_loss_r8_a64_tok_32000_new_format_loss_v1.json', orient='records', lines=True)