...
def preprocess_function(example, tokenizer):
    ...
    model_inputs = tokenizer(example['input_text'], max_length=1800, padding='max_length', truncation=True)
    #tokenizer.padding_side = "right"
    tokenizer.pad_token = tokenizer.eos_token
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        if state.global_step % args.logging_steps == 0:
            if logs:
                ...
                inputs = self.tokenizer(input_text, return_tensors="pt", truncation=True, padding="max_length", max_length=1800).to(self.device)
...
trainer.model.save_pretrained('./fine_tuned_llama2_jigjaw')
tokenizer.save_pretrained('./fine_tuned_llama2_jigjaw')
